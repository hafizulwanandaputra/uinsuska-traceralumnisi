...
<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
    <div class="d-flex justify-content-start align-items-start pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2 me-3"><a href="<?= base_url('/pengumuman-admin/' . $pengumuman['id_pengumuman']); ?>"><i class="fa-solid fa-arrow-left"></i></a></h1>
        <h1 class="h2">Edit "<?= $pengumuman['judul']; ?>"</h1>
    </div>

    <?= form_open_multipart('/pengumuman-admin/update/' . $pengumuman['id_pengumuman']); ?>
    <?= csrf_field(); ?>
    <div class="mb-3 row">
        <label for="judul" class="col-lg-2 col-form-label">Judul</label>
        <div class="col-lg-10">
            <input type="text" class="form-control <?= (validation_show_error('judul')) ? 'is-invalid' : ''; ?>" id="judul" name="judul" value="<?= (old('judul')) ? old('judul') : htmlspecialchars($pengumuman['judul']); ?>" autocomplete="off" autofocus>
    ...
    <div class="mb-3 row">
        <div class="col">
            <textarea id="textarea" class="form-control font-monospace" name="isi" id="isi"><?= (old('isi')) ? old('isi') : $pengumuman['isi']; ?></textarea>
